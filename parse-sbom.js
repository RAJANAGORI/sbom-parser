...
function main() {
  ...
  const ttfDays = Object.values(tracker.vulns).filter(r => r.closedAt).map(r => daysBetween(r.firstSeen, r.closedAt));
  const ttfMedianDays = median(ttfDays);

  const nowISO = now;
  const openAgeDays = Object.entries(tracker.vulns).filter(([k]) => currentKeys.has(k)).map(([,r]) => daysBetween(r.firstSeen, nowISO));
  const openAgeMedianDays = median(openAgeDays);
