...
function readArtifactsList() {
  if (!fs.existsSync(ARTIFACTS_JSON)) {
    console.error(`Missing ${ARTIFACTS_JSON}.`);
    process.exit(1);
  }
  const j = JSON.parse(fs.readFileSync(ARTIFACTS_JSON, "utf8"));
  if (!Array.isArray(j.artifacts)) {
    console.error("artifacts.json: 'artifacts' must be an array.");
    process.exit(1);
  }
  return j.artifacts;
}
...
async function findNewestArtifactByName(name) {
  ...
  for (;;) {
    const { data } = await octokit.actions.listArtifactsForRepo({ owner: OWNER, repo: REPO, per_page: 100, page });
...
async function main() {
  ...
  for (const { name, dataset } of items) {
    if (!name || !dataset) { console.warn("Bad artifacts.json entry:", { name, dataset }); continue; }
